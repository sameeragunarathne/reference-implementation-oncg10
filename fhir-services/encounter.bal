...
service /fhir/r4/Encounter on new fhirr4:Listener(config = encounterApiConfig) {
    ...
    isolated resource function get [string id](r4:FHIRContext fhirContext) returns Encounter|r4:OperationOutcome|r4:FHIRError|error {
        anydata|r4:OperationOutcome|r4:FHIRError|error result;
        lock {
            result = fetchResourceById(fhirContext, "Encounter", id, uscore311:USCoreEncounterProfile);
        }
        if result is Encounter {
            return result;
        }
    ...
    isolated resource function get .(r4:FHIRContext fhirContext) returns r4:Bundle|r4:OperationOutcome|r4:FHIRError|error {
        ...
        lock {
            searchResult = searchResourceBundle(fhirContext, "Encounter");
        }
        return searchResult;
    }
    ...
    isolated resource function post _search(r4:FHIRContext fhirContext) returns r4:Bundle|r4:OperationOutcome|r4:FHIRError|error {
        r4:Bundle|r4:OperationOutcome|r4:FHIRError|error searchResult;
        lock {
            searchResult = searchResourceBundle(fhirContext, "Encounter", mode = fhir:POST);
        }
        return searchResult;
    }
}
